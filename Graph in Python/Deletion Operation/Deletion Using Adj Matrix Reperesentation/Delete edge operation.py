...
def del_edge(v1,v2):
    if v1 not in nodes:
        ...
        graph[index1][index2]=0         # graph[v1][v2]
        graph[index2][index1]=0        # graph[v2][v1]
...
def print_graph():
    for i in range(node_count):
        for j in range(node_count):
            print(format (graph [i] [j], "<3"), end=" ")   # format function use for proper sape of graph.
...
add_edge("A" , "D" , 4)         # connect the edge A - D

add_edge("A" , "C" , 5)             # connect the edge A - C

add_edge("B" , "E" , 3)               # connect the edge B - E

add_edge("B" , "D" , 7)                 # connect the edge B - D

add_edge("D" , "E" , 2)                    # connect the edge D - E

add_edge("D" , "C" , 1)                      # connect the edge D - C
