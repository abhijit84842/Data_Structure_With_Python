# DELETION OPERATION 0 AND 1 CHILD NODE...
...
# In-order Traversal Algo..
    ...
    def inorder(self):
        ...
        if self.lchild:     # if left S.T is present 
            self.lchild.inorder()       # again call rcursively In-order method
        print(self.key, end=" ")
        
        
        if self.rchild:     # if right S.T is present
            self.rchild.preorder()       # again call rcursively IN-order method
        
# pOST ORDERD Taversal
...
# delete operation ...
    ...
    def delete(self,data):
        if self.key is None:        # root node is not present..
            print("Tree is Empty..")
            return
        if data < self.key:
            if self.lchild:
    ...
    #This two condition will delete the node which contain 0 or 1  child nodes 
        else:
...
#print("Preorder")
#root.preorder()     # pre-order traversal algorithm..
